...
export const Route = createFileRoute("/realisations")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Galerie de chantiers premium : marbre, carrelage XXL, terrasses, pierre naturelle." },
      { property: "og:image", content: spaNuit },
    ],
    links: [{ rel: "canonical", href: "https://damienmugnier.fr/realisations" }],
  }),
    ...
    <>
      ...
      <section className="py-20 md:py-28">
        <div className="container-prose columns-1 gap-5 sm:columns-2 lg:columns-3">
          {projects.map((p, i) => (
            <figure key={i} className="group relative mb-5 break-inside-avoid overflow-hidden rounded-2xl">
              <img
                src={p.img}
                alt={p.title}
              ...
              <figcaption className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-5 text-primary-foreground">
                <span className="text-xs font-medium uppercase tracking-widest text-accent">{p.tag}</span>
